...
//3-create as many according to the input
...
submit.onclick = function(){
    let newPro = {
        ...
        provider:provider.value.toUpperCase(),
    }

    if(title.value != '' && price.value != '' && category.value != '' && provider.value != ''){
        if(mood === 'create'){
        if(newPro.count > 1){
...
function updateData(i){
    ...
    provider.value = dataPro[i].provider;
...
function searchData(value){
    ...
    }if(searchMood == 'provider'){
        for(let i = 0; i < dataPro.length; i++){
            if(dataPro[i].provider.includes(value.toUpperCase)){
                table += `
                    <tr>
                    <td>${i}</td>
                    <td>${dataPro[i].title}</td>
